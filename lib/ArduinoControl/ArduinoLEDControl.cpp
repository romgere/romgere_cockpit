...
ArduinoLEDControl::ArduinoLEDControl(uint8_t pin, float highLEDVal,
                                     int boardAddress)
...
ArduinoLEDControl::ArduinoLEDControl(uint8_t pin, float highLEDVal)
  ...
  this->Pin = pin;
  this->Val = -1; // LED will be update next time we will receive other than -1
                  // float value from X-Plane
