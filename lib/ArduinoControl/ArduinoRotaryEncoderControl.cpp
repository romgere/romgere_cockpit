...
For all types of rotary encoder, the direction logic is the same. Only detents
vary. Command are send only if rotary is stop on a detent.
*/
#ifdef ACTIVE_MULTI_ARDUINO_BOARD_MODE
ArduinoRotaryEncoderControl::ArduinoRotaryEncoderControl(uint8_t pin1,
                                                         uint8_t pin2,
...
ArduinoRotaryEncoderControl::ArduinoRotaryEncoderControl(uint8_t pin1,
  ...
  this->LastPinStatus[ROTARY_ENC_CUR_STATUS_INDEX] =
      this->LastPinStatus[ROTARY_ENC_OLD_STATUS_INDEX] = 0;

  bitWrite(this->LastPinStatus[ROTARY_ENC_CUR_STATUS_INDEX], 0,
           _digitalRead(pin1) == HIGH);
  bitWrite(this->LastPinStatus[ROTARY_ENC_CUR_STATUS_INDEX], 1,
           _digitalRead(pin2) == HIGH);

  this->LastPinStatus[ROTARY_ENC_OLD_STATUS_INDEX] =
      this->LastPinStatus[ROTARY_ENC_CUR_STATUS_INDEX];
...
bool ArduinoRotaryEncoderControl::ReadInput() {
  ...
  bool isStatusChanged =
      this->LastPinStatus[ROTARY_ENC_OLD_STATUS_INDEX] != newVal;
...
//"Secure“ mode (for cheap type 3 mouse encoder)
  ...
  if (EncoderType == Type3Encoder) {
    ...
    if (this->LastPinStatus[ROTARY_ENC_OLD_STATUS_INDEX] == 0 && newVal == 3) {
      // Serial.print("Control Debug : Check RotaryEncoder, impossible stat 0 to
      // 3 !");
      return false;
    }
    // Current 1 => allow only new stat to 0 or 3
    else if (this->LastPinStatus[ROTARY_ENC_OLD_STATUS_INDEX] == 1 &&
             newVal == 2) {
      // Serial.print("Control Debug : Check RotaryEncoder, impossible stat 1 to
      // 2 !");
      return false;
    }
    // Current 2 => allow only new stat to 0 or 3
    else if (this->LastPinStatus[ROTARY_ENC_OLD_STATUS_INDEX] == 2 &&
             newVal == 1) {
      // Serial.print("Control Debug : Check RotaryEncoder, impossible stat 2 to
      // 1 !");
      return false;
    }
    // Current 3 => allow only new stat to 1 or 2
    else if (this->LastPinStatus[ROTARY_ENC_OLD_STATUS_INDEX] == 3 &&
             newVal == 0) {
      // Serial.print("Control Debug : Check RotaryEncoder, impossible stat 3 to
      // 0 !");
      return false;
    }
  }
  ...
  // According to the encoder type, somes states does not corresponding to a
  // detent, return false in these case.
  switch (EncoderType) {
  ...
  case Type2Encoder:
    if (this->LastPinStatus[ROTARY_ENC_CUR_STATUS_INDEX] == 1 ||
        this->LastPinStatus[ROTARY_ENC_CUR_STATUS_INDEX] == 2)
      return isStatusChanged;
    else
      return false;
  ...
  case Type3Encoder:
    ...
    if ((this->LastPinStatus[ROTARY_ENC_CUR_STATUS_INDEX] == 0 &&
         this->LastPinStatus[ROTARY_ENC_OLD_STATUS_INDEX] != 3) ||
        (this->LastPinStatus[ROTARY_ENC_CUR_STATUS_INDEX] == 3 &&
         this->LastPinStatus[ROTARY_ENC_OLD_STATUS_INDEX] != 0))
      return isStatusChanged;
#else
    if (this->LastPinStatus[ROTARY_ENC_CUR_STATUS_INDEX] == 0 ||
...
float ArduinoRotaryEncoderControl::getValue() {
  ...
  case 0:
    curDirection =
        (this->LastPinStatus[ROTARY_ENC_OLD_STATUS_INDEX] == 1 ? UpDirection
                                                               : DownDirection);
    break;
  case 1:
    curDirection =
        (this->LastPinStatus[ROTARY_ENC_OLD_STATUS_INDEX] == 3 ? UpDirection
                                                               : DownDirection);
    break;
  case 2:
    curDirection =
        (this->LastPinStatus[ROTARY_ENC_OLD_STATUS_INDEX] == 0 ? UpDirection
                                                               : DownDirection);
    break;
  case 3:
    curDirection =
        (this->LastPinStatus[ROTARY_ENC_OLD_STATUS_INDEX] == 2 ? UpDirection
                                                               : DownDirection);
    break;
  }
