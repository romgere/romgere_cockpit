...
void MasterToSlaveCommand::SendDataToIC2(uint8_t boardAddress) {
  ...
  if (this->TypeCommande != TypeCommandSetPINValue &&
...
// Send a "GetPIN" command to slave board and wait return of slave board on I2C
// bus
int MasterToSlaveCommand::RequestDataFromIC2(uint8_t boardAddress) {
  ...
  // We can't do this because one I2C transaction must include write,
  // requestFrom and read actions SendDataToIC2( boardAddress);
