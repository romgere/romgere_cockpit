...
void CockpitMainApplication::RegisterInputControl(ArduinoInputControl *ctrl,
  ...
  */
  ...
  // Additional command for multiple position input control (rotary switch,
  // toggle switch ...)
  ...
  Serial.print("CockpitMainApplication : Register INPUT control/command "
               "association, control : [");
...
#ifdef DEBUG_CONTROL_COMMAND_REGISTER
  Serial.print("CockpitMainApplication : Register OUTPUT control/data "
               "association, control : [");
...
#ifdef DEBUG_LIBRARY_LOOP
    Serial.println(
        "CockpitMainApplication : Loop, process Input Data Reading...");
#endif
...
#ifdef DEBUG_LIBRARY_LOOP
    Serial.println(
        "CockpitMainApplication : Loop, process Input Control Reading...");
#endif
...
void CockpitMainApplication::updateControlWithCommand(
  ...
  if (dataXplane != NULL) {
...
#ifdef DEBUG_LIBRARY_LOOP
    Serial.print("CockpitMainApplication : Loop, data received for control, "
                 "(new) value :");
#endif
    ...
    Serial.println(
        "CockpitMainApplication : Loop, NO data received for control.");
  }
#endif
}
...
void CockpitMainApplication::doControlCommandProcess(
  ...
  if (forceSendingCommand || inputControl->ReadInput()) {
...
#ifdef DEBUG_LIBRARY_LOOP
    Serial.print(
        "CockpitMainApplication : Loop, control was modified, new value:");
    Serial.print(val);
    Serial.println("...");
#endif
...
#ifdef DEBUG_LIBRARY_LOOP
      Serial.println("CockpitMainApplication : Loop, control type Analogic, "
                     "not implemented!");
#endif
    }
    ...
    if (outputCmd != NULL) {
...
#ifdef DEBUG_LIBRARY_LOOP
          Serial.print(
              "CockpitMainApplication : Loop, Process SECOND Command Simple [");
      ...
      case XPlaneOutputCommand::TypeDATACommand:
#ifdef DEBUG_LIBRARY_LOOP
        Serial.println("CockpitMainApplication : Loop, Command type DATA or "
                       "DREF for control, not implemented!");
#endif
        break;
    ...
    // No command
      ...
      Serial.println(
          "CockpitMainApplication : Loop, no command for control value.");
    }
#endif
  }
#ifdef DEBUG_LIBRARY_LOOP
