...
// Keep a global reference to SlaveBoardApplication instance to be able to
// notify it
SlaveBoardApplication *slaveAppInstance = NULL;
void globalReceivedData(int dataSize) {
  slaveAppInstance->receivedData(dataSize);
}
void globalRequestEvent() { slaveAppInstance->requestEvent(); }

SlaveBoardApplication::SlaveBoardApplication(uint8_t adresse)
    : BoardI2CAdresse(adresse), lastPinForValueSend(NULL) {
  ...
  while (!this->commandQueue.isEmpty()) {
    ...
    if (masterCommand->TypeCommande ==
      ...
      if (masterCommand->PinMode == MasterToSlaveCommand::PINModeNotUsed) {
...
#ifdef DEBUG_SLAVE_APP
        Serial.println(
            "SlaveBoardApplication : Init PIN Erreur, bad parameter.");
#endif
        ...
        Serial.print(",");
        Serial.print(
            masterCommand->PinMode == MasterToSlaveCommand::PINModeInput ? INPUT
            : masterCommand->PinMode ==
                    MasterToSlaveCommand::PINModeIntputPullUp
                ? INPUT_PULLUP
                : OUTPUT);
        Serial.println(").");
#endif

        pinMode(masterCommand->PinNum,
                masterCommand->PinMode == MasterToSlaveCommand::PINModeInput
                    ? INPUT
                : masterCommand->PinMode ==
                        MasterToSlaveCommand::PINModeIntputPullUp
                    ? INPUT_PULLUP
    ...
    else if (masterCommand->TypeCommande ==
             MasterToSlaveCommand::TypeCommandSetPINValue) {
...
#ifdef DEBUG_SLAVE_APP
        Serial.println(
            "SlaveBoardApplication : Set PIN Erreur, bad parameter.");
#endif
...
void SlaveBoardApplication::parseDataFromMaster(byte *data) {
  ...
  if (masterCommand->TypeCommande ==
      MasterToSlaveCommand::TypeCommandGetPINValue) {
    ...
    // Keep the command (reference to arduino input PIN) to send it later when
    // master board really ask for it (see
    // "SlaveBoardApplication::requestEvent()")
...
void SlaveBoardApplication::requestEvent() {
  ...
  if (lastPinForValueSend != NULL) {
    ...
    if (lastPinForValueSend->TypeCommande ==
            MasterToSlaveCommand::TypeCommandGetPINValue &&
        (lastPinForValueSend->PinMode == MasterToSlaveCommand::PINModeInput ||
         lastPinForValueSend->PinMode ==
             MasterToSlaveCommand::PINModeIntputPullUp)) {
      ...
      if (lastPinForValueSend->RWMode == MasterToSlaveCommand::RWModeAnalog) {
        ...
        Serial.println(");");
#endif
      } else if (lastPinForValueSend->RWMode ==
